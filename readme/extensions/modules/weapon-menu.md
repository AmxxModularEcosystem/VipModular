...
| ResetCountOnSpawn     | Нет           | `true`/`false`                                 | Сбрасывать ли счётчик предметов при спавне, иначе в начале раунда. По умолчанию `false`                          |
...
| InactiveTitle | Нет          | Строка                                         | Текст пункта в оружейном меню, когда пункт неактивен. По умолчанию используется `Title`      |
...
Все отображаемые строки модуля поддерживают плейсхолдеры: `MainMenuTitle`, а также `Name`, `Title` и `FakeMessage` меню и `Title`/`InactiveTitle` пунктов.

Доступны плейсхолдеры глобальной группы (`{map}`, `{time}`, `{server-name}`, ...) и группы игрока с префиксом `p` (`{p:name}`, `{p:frags}`, `{p:health}`, ...). Плейсхолдеры подставляются в момент показа меню с контекстом игрока, которому оно показывается. Неизвестные плейсхолдеры остаются в строке как есть.

```jsonc
{
    "Title": "Меню игрока {p:name}",
    "Items": [
        { "Title": "{p:name}, осталось убийств: {p:frags}", "Items": "File:Items/Deagle" }
    ]
}
```

Полный список доступных плейсхолдеров — в документации [ParamsController](https://github.com/AmxxModularEcosystem/ParamsController).
