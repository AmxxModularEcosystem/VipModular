...
> Типы параметров в таблицах (`Integer`, `Boolean`, `String`, `VipM-Limits`, `IC-Items`, …) описаны в скиллах `param-types`, `vipm-core-param-types`, `vipm-ic-param-types`.
...
- Типы без параметров (`DefuseKit`, `InstantReload`, …) принимают только сам
  объект с `"Item"`.
- У предметов **есть** список параметров (его задаёт тип предмета); отсутствие обязательного параметра или неизвестное имя `"Item"` → предмет отбрасывается.
