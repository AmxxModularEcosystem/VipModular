...
| `InactiveTitle` | String | текст пункта, когда он неактивен (default — `Title`) |
...
> **Плейсхолдеры.** Все отображаемые строки — `MainMenuTitle`, `Name`, `Title`,
> `FakeMessage` меню и `Title`/`InactiveTitle` пунктов — поддерживают плейсхолдеры:
> глобальные (`{map}`, `{time}`, ...) и игрока с префиксом `p` (`{p:name}`,
> `{p:frags}`, ...). Подставляются при показе меню с контекстом игрока.
